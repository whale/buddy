...
| **Mac⇄iOS sync** | `feat/sync-server-merge` (step 4, draft) + plan | Design done. **Steps 1–3 ✅** (UUIDs; per-item `v` + tombstones + `erasedAt` + idempotent rollover; pure `merge()` BOTH apps + Mac boot-reconcile wiring — Mac mergeTest 12/12 + smokeTest 11/11, iOS XCTest 10/10). **Step 4 🟡 WRITTEN, NOT VERIFIED** — server merge-on-push migration + ts-unit normalization + empty-over-full guard + rate limit + test harness, all on `feat/sync-server-merge`. **BLOCKED on a live DB** (no Postgres locally; start OrbStack → `supabase start` → run `supabase/tests/buddy_merge_on_push_test.sql`). Steps 5–6 in `IOS-COMPANION-PLAN.md`. |
