...
--
-- This is the five migrations (buddy_state + CAS + hardening + throttle fix + pull stats) concatenated, in order.
-- ============================================================================
...
-- ============================================================================
-- WIRE FLOOR (SYNC-COMPAT.md) — appended so this is the FINAL buddy_push
-- definition (create-or-replace: last one wins). Keep in lockstep with
-- supabase/migrations/20260719130000_buddy_wire_floor_fix.sql. Ships DISABLED
-- (floor 0 = no-op); raise via buddy_set_wire_floor(2) AFTER the wire-2 build
-- saturates. Only the SERVER can stop a frozen pre-header client from clobbering.
-- ============================================================================
...
-- The 5-arg buddy_push (throttle version) + the floor check. This RE-DEFINES the
-- function defined earlier in this file, adding only the v_floor block.
create or replace function public.buddy_push(
  p_key text, p_blob jsonb, p_expected bigint,
  p_device text default null, p_stats jsonb default null
)
...
begin
  ...
  -- WIRE FLOOR: reject a write whose cleartext wire is below the floor (frozen old client).
